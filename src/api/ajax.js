        ...
        if (String(res.code) === '200' || String(res.code) === '0') {
            ...
            if (String(stateCode.code) === '200' || String(stateCode.code) === '0') {
                ...
                if (String(stateCode.code) !== '405')
                    Message.error(config.errorMsg || stateCode.msg || stateCode.desc || res.desc || String(response.status));
...
export function createApi(args, format, request = instance) {
    return function (target, name) {
        const oldValue = target[name];
        target[name] = async function (data = {}, showMsg, errorMsg) {
